...
### Phase 1: Foundation
**Goal:** Users can install a skill or MCP server by name targeting Claude Code, see what is installed, manage registry sources, and uninstall cleanly — using a locally-built binary with no runtime dependency.
**Mode:** mvp
**Depends on:** Nothing
**Requirements:** CLI-01, CLI-02, CLI-05, CLI-06, CLI-07, CLI-08, CLI-09, REG-01, REG-02, REG-05, REG-06, AST-01, MCP-01, MCP-03, MCP-05, MCP-06, MCP-07, SKL-01, SKL-02, SKL-03
**Success Criteria**:
  1. `agentkit install playwright --target claude` resolves the package from the default registry, runs the npx or binary install adapter, writes the correct MCP entry to `~/.claude/settings.json` (or runtime-detected path), and prints a success line with version and target.
  ...
  3. `agentkit search playwright` returns ranked results from all registered registries with source labels.
  4. `agentkit uninstall playwright` removes the skill or MCP entry with no leftover artifacts.
  5. `agentkit registry add <url>` and `agentkit registry remove <url>` persist the change; subsequent `agentkit install` resolves packages from the new source.
**Plans**: TBD
...
| 2. Multi-Assistant & Full Install | 0/2 | Not started | - |
| 3. Bundled Skills | 0/2 | Not started | - |
| 4. Distribution & Hardening | 0/1 | Not started | - |
